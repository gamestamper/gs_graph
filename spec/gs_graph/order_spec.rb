...
describe GSGraph::Order do
  subject { order }
  let(:order) { GSGraph::Order.new attributes[:id], attributes }
  let :attributes do
    {
      :id => '9005539514450',
      :from => {
  ...
  its(:from)         { should == GSGraph::User.new('579612276', :name => 'Nov Matake') }
  its(:to)           { should == GSGraph::User.new('579612276', :name => 'Nov Matake') }
  its(:application)  { should == GSGraph::Application.new('134145643294322', :name => 'gem sample') }
  its(:amount)       { should == 2 }
