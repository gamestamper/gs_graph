...
describe GSGraph::Connections::AdGroups, '#ad_groups' do
  context 'when included by GSGraph::AdCampaign' do
    context 'when access_token is given' do
      it 'should return ad_groups as GSGraph::AdGroup' do
        mock_graph :get, '22334455/adgroups', 'ad_campaigns/ad_groups/22334455_ad_groups', :access_token => 'access_token' do
          ad_groups = GSGraph::AdCampaign.new('22334455', :access_token => 'access_token').ad_groups
          ad_groups.size.should == 2
          ad_groups.each { |ad_group| ad_group.should be_instance_of(GSGraph::AdGroup) }
          ad_groups.first.should == GSGraph::AdGroup.new(
            "6003570493888",
...
describe GSGraph::Connections::AdGroups, '#ad_group!' do
  context 'when included by GSGraph::AdAccount' do
    it 'should return generated ad_group' do
      mock_graph :post, 'act_22334455/adgroups', 'ad_accounts/ad_groups/post_with_valid_access_token' do
        ad_group = GSGraph::AdAccount.new('act_22334455', :access_token => 'valid').ad_group!(
          :name => "Test Ad 1",
